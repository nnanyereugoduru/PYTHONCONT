...
## Live website (recommended for readability)
...
This starts a small local server with a search box and left/right/undetermined
filters. It's stdlib-only — no extra dependency beyond `feedparser` — and it
**imports `bias_tracker.py` directly** rather than reimplementing anything, so
the site always reflects exactly what the underlying code does. If you change
clustering, bias tagging, or blind-spot logic in `bias_tracker.py`, the
website picks it up automatically on the next request.

The static HTML export (`--format html`) and the live site share the same
`render_html()` function, so a saved report and the live page always look
identical.
